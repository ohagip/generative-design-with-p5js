const Agent = function(noiseStickingRange, agentAlpha, noiseScale, noiseStrength, strokeWidth, agentWidthMin, agentWidthMax, zNoiseVelocity) {
...
Agent.prototype.updateStart = function() {
  ...
  if (this.vector.x < -10) {
    this.vector.x = this.vectorOld.x = myp5.width + 10;
  }
  if (this.vector.x > myp5.width + 10) {
    this.vector.x = this.vectorOld.x = -10;
  }
  if (this.vector.y < -10) {
    this.vector.y = this.vectorOld.y = myp5.height + 10;
  }
  if (this.vector.y > myp5.height + 10) {
...
Agent.prototype.update1 = function() {
  ...
  const agentWidth = myp5.lerp(this.agentWidthMin, this.agentWidthMax, this.randomizer);
  myp5.push();
